...
unsigned producir_dato()
{
  ...
  sem_wait( &mutex);
  cout << "producido: " << contador << endl << flush ;	//Muestra antes de aumentar
  sem_post( &mutex);
  return ++contador;
}
// ---------------------------------------------------------------------
...
{  
	...
	for( unsigned i = 0 ; i < num_items ; i=i+10 ){ 
   	...
   	primera_libre %= 10;		//Lo reiniciamos
...
{  
    ...
    for( int i=0; i < tam_vector; i++){
    	dato = vector_datos[primera_ocupada];
    	primera_ocupada++;
    	sem_wait( &mutex);
    	consumir_dato( dato );
    	sem_post( &mutex);
    ...
    primera_ocupada %= 10;		//Lo reiniciamos
